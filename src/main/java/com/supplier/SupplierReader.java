...
public class SupplierReader {
     ...
     * @return A stream of {@link ApiTestDataSupplier} objects.
     */
    @DataSupplier(runInParallel = true)
    public static StreamEx<ApiTestDataSupplier> getDataFromExcel(Method method) {
        return use(XlsxReader.class)
                .withTarget(ApiTestDataSupplier.class)
                .withSource("TestingFile.xlsx")
                .read()
                .filter(apiTestDataSupplier -> apiTestDataSupplier.isExecute() && apiTestDataSupplier.getTestcase().equalsIgnoreCase(method.getName()));
    }
